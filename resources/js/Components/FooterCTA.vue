...
defineProps({
    ...
    buttonText: { type: String, default: "Chat Langsung via WhatsApp" },
    whatsappLink: { type: String, required: true },
    // Skip the outer section/max-width wrapper when the CTA is already
    // embedded inside a parent section (e.g. Index.vue listing pages).
    bare: { type: Boolean, default: false },
});
</script>

<template>
    <div v-if="bare" class="relative overflow-hidden rounded-2xl bg-[#9e1f16] px-6 py-12 sm:px-10 sm:py-14">
        <img src="/icons/ic-footer.png" alt=""
    ...
    <section v-else class="bg-[#F7F7F5] mb-12 sm:mb-16">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="relative overflow-hidden rounded-2xl bg-[#9e1f16] px-6 py-12 sm:px-10 sm:py-14">
                <img src="/icons/ic-footer.png" alt=""
                    class="pointer-events-none absolute right-6 top-1/2 h-18 w-auto -translate-y-1/2 sm:right-10 sm:h-30" />
                <div class="relative flex flex-col items-center text-center">
                    <h3
