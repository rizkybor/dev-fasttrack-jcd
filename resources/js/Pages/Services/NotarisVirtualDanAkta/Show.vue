                ...
                <div class="grid gap-8 lg:grid-cols-[1fr_280px] xl:grid-cols-[1fr_300px] min-w-0">
                    ...
                    <div class="flex flex-col gap-5 min-w-0">
                        <template v-if="selectedPaket">
                            ...
                            <!-- 3a. Daftar Layanan (mode accordion, product id 1 & 2) — toggle list + -->
                            <!-- detail item yang dibuka (Penjelasan Detail, Paket & Harga, Dokumen Diperlukan) -->
                            <!-- tetap dalam 1 card yang sama seperti styling awal, terpisah dari card Penjelasan Layanan -->
                            <div v-if="isAccordionMode && selectedPaket.penjelasan_layanan_items?.length"
                                class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                                <div class="flex items-center gap-3 mb-5">
                                    <img src="/icons/ic-menu-arrow.svg" class="w-6 h-6" alt="" />
                                    <h2 class="text-[15px] font-bold uppercase tracking-widest text-black">
                                        {{ t("services.notarisVirtualDanAktaDetail.sections.daftar_layanan") }}
                                    </h2>
                                </div>
                            ...
                            <!-- 5. Paket & Harga (mode flat) -->
                            <template v-if="!isAccordionMode && selectedPaket.penjelasan_layanan_items?.length">
                                <div v-for="item in selectedPaket.penjelasan_layanan_items" :key="`paket-${item.id}`">
                                    <div v-if="item.paket_harga"
                                        class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                                        <div class="flex items-center gap-3 mb-5">
                                            <img src="/icons/ic-menu-arrow.svg" class="w-6 h-6" alt="" />
                            ...
                            <!-- 6. Dokumen Diperlukan (mode flat) -->
                            <template v-if="!isAccordionMode && selectedPaket.penjelasan_layanan_items?.length">
                                <div v-for="item in selectedPaket.penjelasan_layanan_items" :key="`dok-${item.id}`">
                                    <div v-if="item.dokumen_diperlukan?.length"
                                        class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                                        <div class="flex items-center gap-3 mb-5">
                                            <img src="/icons/ic-menu-arrow.svg" class="w-6 h-6" alt="" />
