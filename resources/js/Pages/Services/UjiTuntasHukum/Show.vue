    ...
    <MainLayout>
        ...
        <section class="relative overflow-hidden min-h-[280px] sm:min-h-[320px] lg:min-h-[360px]">
            <div class="absolute inset-0">
                <img
                    src="/images/layanan-hero/ft-hero-uji-tuntas-hukum.png"
            ...
            <div class="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-16 flex flex-col justify-between h-full min-h-[280px] sm:min-h-[320px] lg:min-h-[360px]">
                ...
                <div>
                    <a href="/layanan" class="inline-flex items-center gap-2 text-sm font-semibold text-white hover:text-white/70 transition">
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <div v-if="mengapaPenting" class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                            ...
                            <ul class="space-y-2.5 mb-4">
                                <li
                                    v-for="(item, i) in mengapaPenting.items"
                                    :key="`penting-${i}`"
                        ...
                        <div v-if="ruangLingkup" class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                                <div
                                    v-for="(cat, ci) in ruangLingkup.categories"
                                    :key="`cat-${ci}`"
                                >
                                    ...
                                    <ul class="space-y-1">
                                        ...
                                        >
                                            <span class="mt-[5px] w-1.5 h-1.5 rounded-full bg-[#9e1f16] flex-shrink-0"></span>
                                            {{ item }}
                                        </li>
                                    </ul>
                                </div>
                        ...
                        <div v-if="direkomendasikan" class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                            ...
                            <div class="flex flex-col gap-4">
                                <div
                                    v-for="(item, i) in direkomendasikan.items"
                                    :key="`direk-${i}`"
                                    class="flex items-start gap-4"
                                >
                                    <div class="flex-shrink-0 w-8 h-8 rounded-full bg-[#9e1f16] flex items-center justify-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-[13px] text-[#686964] leading-[1.6]">{{ item.description }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="keunggulan" class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div
                                    v-for="(item, i) in keunggulan.items"
                                    :key="`unggulan-${i}`"
                                ...
                                >
                                    ...
                                    <div>
                                        <div class="text-[13px] font-bold text-[#1A1B18] mb-0.5">{{ item.title }}</div>
                                        <div class="text-[12px] text-[#686964] leading-[1.6]">{{ item.description }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            <div class="flex items-center justify-between px-5 py-4 sm:px-6 sm:py-5 gap-4">
                                <div class="flex items-center gap-4">
                                    <div class="flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-white/15 flex items-center justify-center">
                                        <svg class="w-6 h-6 sm:w-7 sm:h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                                        </svg>
                                    </div>
                                    <p class="text-[12px] sm:text-[13px] text-white/90 leading-[1.6] max-w-sm">
                                        {{ bannerCta.text }}
                                    </p>
                                </div>
                                <a
