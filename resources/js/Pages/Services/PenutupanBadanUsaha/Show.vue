...
const activeBiayaTabData = computed(
    () => props.product?.biaya_layanan_tabs?.[activeBiayaTab.value] ?? {}
);
</script>
...
<div class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
    <div class="flex items-center justify-between mb-5">
        ...
        <div v-if="product.biaya_layanan_tabs?.length"
            class="inline-flex rounded-lg border border-[#E8E8E6] overflow-hidden">
            <button
                v-for="(tab, ti) in product.biaya_layanan_tabs"
                :key="`tab-${ti}`"
    ...
    <template v-if="product.biaya_layanan_tabs?.length">
    ...
    <template v-else-if="product.biaya_layanan_single">
        <div class="rounded-xl border border-[#E8E8E6] p-5">
            <p class="text-[13px] font-bold text-[#1A1B18] mb-1">{{ product.biaya_layanan_single.nama }}</p>
            <p class="text-[11px] text-[#686964] mb-0.5">Mulai dari</p>
            <p class="text-[24px] font-bold text-primary leading-tight mb-2">{{ product.biaya_layanan_single.harga }}</p>
            <div v-if="product.biaya_layanan_single.gratis_konsultasi" class="flex items-center gap-1.5 mb-4">
                <img src="/icons/ft-done.svg" class="h-4 w-4 flex-shrink-0" alt="" />
    ...
    <template v-else-if="product.biaya_layanan_cards?.length">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div v-for="(card, ci) in product.biaya_layanan_cards" :key="`blc-${ci}`"
                    ...
                    <div class="flex flex-col gap-4 lg:sticky lg:top-32 lg:self-start">
                        ...
                        <div class="rounded-2xl border border-[#E8E8E6] bg-white p-5">
                            <div class="mb-3 inline-flex items-center gap-1.5 rounded-full bg-[#FFF0EF] px-3 py-1">
                                ...
                                <span class="text-[11px] font-semibold text-primary truncate max-w-[160px]">{{ product.name }}</span>
                            </div>
                            <div class="text-[12px] text-[#686964] mb-1 mt-2">Start From</div>
                            <div class="text-[32px] font-bold leading-none text-primary mb-1">{{ product.price_label }}</div>
