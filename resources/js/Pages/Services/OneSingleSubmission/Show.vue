            ...
            <div
                ...
                <nav aria-label="Breadcrumb">
                    <div class="inline-flex items-center gap-2 rounded-md bg-white px-4 py-2">
                        ...
                        <a href="/one-single-submission" class="text-sm font-medium text-[#9e1f16] hover:underline">One
                            Single Submission</a>
                        <svg class="h-3 w-3 text-[#9e1f16]" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                            stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                    ...
                    <div class="space-y-4 text-[14px] leading-[1.8] text-[#3D3D3A] text-justify">
                        ...
                        <template v-if="product.penjelasan_umum[1]">
                            ...
                            <ul v-if="product.penjelasan_umum_bullets?.length"
                                ...
                                <li v-for="(b, i) in product.penjelasan_umum_bullets" :key="`b1-${i}`">{{ b }}
                                </li>
                            </ul>
                        </template>
                        ...
                        <template v-if="product.penjelasan_umum[2]">
                            ...
                            <ul v-if="product.penjelasan_umum_bullets_2?.length"
                                class="list-disc list-inside space-y-1 pl-1">
                                <li v-for="(b, i) in product.penjelasan_umum_bullets_2" :key="`b2-${i}`">{{ b }}
                                </li>
                            </ul>
                        </template>

                        <p v-if="product.penjelasan_umum[3]">{{ product.penjelasan_umum[3] }}</p>
                    </div>
                </div>
                ...
                <div v-if="hasPaket" class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8 mb-5">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <button v-for="paket in product.paket" :key="paket.id" @click="selectedPaketId = paket.id"
                            ...
                            <p class="text-[11px] leading-[1.5] mb-4"
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <template v-if="selectedPaket">
                            ...
                            <div v-if="selectedPaket.dokumen_legalitas"
                                ...
                                <!-- Dokumen -->
                                <div v-if="selectedPaket.dokumen_legalitas.dokumen?.length" class="mb-5">
                                    <p class="text-[13px] font-bold text-[#1A1B18] mb-3">Dokumen</p>
                                    <ul class="space-y-2.5">
                                        <li v-for="(item, i) in selectedPaket.dokumen_legalitas.dokumen"
                                            :key="`dok-${i}`" class="flex items-start gap-2.5">
                                ...
                                <!-- Termasuk -->
                                <div v-if="selectedPaket.dokumen_legalitas.termasuk?.length">
                                    <p class="text-[13px] font-bold text-[#1A1B18] mb-3">Termasuk</p>
                                    <ul class="space-y-2.5">
                                        <li v-for="(item, i) in selectedPaket.dokumen_legalitas.termasuk"
                                            :key="`tmsk-${i}`" class="flex items-start gap-2.5">
                                            <img src="/icons/ft-done.svg" class="mt-0.5 h-4 w-4 flex-shrink-0"
                                                alt="done" />
                                            <span class="text-[13px] leading-[1.6] text-[#3D3D3A]">{{ item }}</span>
                                        </li>
                            ...
                            <!-- 5. Biaya Layanan -->
                            <div v-if="selectedPaket.biaya_layanan"
                                class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                                <div class="flex items-center gap-3 mb-5">
                                    <img src="/icons/ic-menu-arrow.svg" class="w-6 h-6" alt="" />
                                    <h2 class="text-[15px] font-bold uppercase tracking-widest text-black">
                                ...
                                <button v-for="paket in product.paket.filter(p => p.id !== selectedPaketId)"
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[12px] font-bold text-primary">{{ paket.harga }}</p>
                                        </div>
                                    </div>
