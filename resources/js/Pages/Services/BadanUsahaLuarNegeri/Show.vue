...
<template>
    <MainLayout>
            ...
            <div
                ...
                <nav aria-label="Breadcrumb">
                    <div class="hidden sm:inline-flex items-center gap-2 rounded-md bg-white px-4 py-2">
                        ...
                        <a href="/badan-usaha-luar-negeri" class="text-sm font-medium text-[#9e1f16] hover:underline">
                            {{
                                t(
                ...
                <!-- Bottom: Back button -->
                <div>
                    <a href="/badan-usaha-luar-negeri"
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid gap-8 lg:grid-cols-[1fr_280px] xl:grid-cols-[1fr_300px] min-w-0">
                    ...
                    <div class="flex flex-col gap-6 min-w-0">
                        ...
                        <!-- 2. Kriteria -->
                        <div v-if="currentData.kriteria?.length"
                            class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                            <div class="flex items-center gap-3 mb-5">
                                <img src="/icons/ic-menu-arrow.svg" class="w-6 h-6" alt="" />
                                <h2 class="text-[15px] font-bold uppercase tracking-widest text-black">
                                    {{
                                        t(
                                            "services.badanUsahaLuarNegeriDetail.sections.kriteria",
                                        )
                                    }}
                                </h2>
                            </div>
                            ...
                            <ol class="space-y-5">
                                <li v-for="(item, kIndex) in currentData.kriteria" :key="`kriteria-${kIndex}`">
                                    <div class="flex gap-2.5">
                                        <span class="flex-shrink-0 text-[13px] font-semibold text-[#1A1B18]">
                                            {{ kIndex + 1 }}.
                                        </span>
                                        <span class="text-[13px] font-semibold leading-[1.6] text-[#1A1B18]">
                                            {{ item.text }}
                                        </span>
                                    </div>

                                    <ul v-if="item.items?.length" class="mt-2 ml-6 space-y-1.5">
                                        <li v-for="(point, iIndex) in item.items"
                                    ...
                                    <div v-if="item.note" class="mt-3 ml-6">
                                        <p class="text-[13px] leading-[1.6] text-[#3D3D3A] mb-1.5">
                                            {{ item.note.label }}
                                        </p>
                                        <ul class="space-y-1.5">
                                            <li v-for="(point, nIndex) in item.note.items"
                        ...
                        <!-- 3. Paket & Harga -->
                        <div v-if="currentData.paket_harga"
                            class="rounded-2xl border border-[#E8E8E6] bg-white p-6 sm:p-8">
                            <div class="flex items-center gap-3 mb-5">
                                <img src="/icons/ic-menu-arrow.svg" class="w-6 h-6" alt="" />
                                <h2 class="text-[15px] font-bold uppercase tracking-widest text-black">
                            ...
                            <div class="rounded-2xl border border-[#E8E8E6] overflow-hidden">
                                <!-- Harga -->
                                <div class="p-5 sm:p-6">
                                    <p class="text-[13px] font-bold uppercase tracking-wide text-[#1A1B18]">
                                    ...
                                    <p class="text-[24px] font-bold text-primary leading-tight">
                                        {{ currentData.paket_harga.harga }}
                                    </p>
                                    <div v-if="currentData.paket_harga.bonus" class="flex items-start gap-2 mt-3">
                                        <img src="/icons/ft-done.svg" class="mt-0.5 h-3.5 w-3.5 flex-shrink-0" alt="" />
                                        <span class="text-[12px] leading-[1.5] text-[#686964]">
                                ...
                                <!-- Rincian paket -->
                                <div class="border-t border-[#E8E8E6] p-5 sm:p-6 space-y-5">
                                    <div v-if="currentData.paket_harga.dokumen_legalitas?.length">
                                        ...
                                        <ul class="space-y-2.5">
                                            <li v-for="(item, index) in currentData.paket_harga.dokumen_legalitas"
                                                :key="`legalitas-${index}`" class="flex items-start gap-2.5">
                                    ...
                                    <div v-if="currentData.paket_harga.termasuk?.length">
                                        <p class="text-[13px] font-bold text-[#1A1B18] mb-3">
                                            {{
                                                t(
                                        ...
                                        <ul class="space-y-2.5">
                                            <li v-for="(item, index) in currentData.paket_harga.termasuk"
                                                :key="`termasuk-${index}`" class="flex items-start gap-2.5">
                                                <img src="/icons/ft-done.svg" class="mt-0.5 h-4 w-4 flex-shrink-0"
                                                    alt="done" />
                                                <span class="text-[13px] leading-[1.6] text-[#3D3D3A]">{{ item }}</span>
                                            </li>
                                    ...
                                    <a :href="buildWhatsappLink(localizedProduct.name)" target="_blank"
                        ...
                        <div v-if="currentRequirements.length"
                            ...
                            <div v-show="docsOpen" class="px-6 sm:px-8 pb-6 sm:pb-8 border-t border-[#E8E8E6]">
                                <ol class="mt-5 space-y-4">
                                    <li v-for="(req, index) in currentRequirements" :key="`doc-${index}`"
                                        class="flex gap-4">
                                        <!-- Nomor -->
                                        <span
                                        ...
                                        <div class="pt-0.5 flex-1">
                                            ...
                                            <div v-if="req.groups && !req.sections" class="mt-1 space-y-2">
                                                <div v-for="(group, gIndex) in req.groups" :key="`group-${gIndex}`">
                                                    ...
                                                    <ul v-if="group.items"
                                                        class="mt-0.5 space-y-0.5 list-disc list-inside">
                                                        <li v-for="(item, iIndex) in group.items"
        ...
        <FooterCTA :title="t('services.badanUsahaLuarNegeriDetail.footer.title')"
