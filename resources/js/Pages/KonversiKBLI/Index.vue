...
onMounted(async () => {
    try {
        const res = await fetch("/data/tabel-konversi-kbli2020-x-kbli2025.json");
    ...
    <MainLayout>
            ...
            <div class="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14 flex flex-col justify-between h-full min-h-[280px] sm:min-h-[320px]">
                ...
                <nav aria-label="Breadcrumb">
                    <div class="inline-flex items-center gap-2 rounded-md bg-white px-4 py-2">
                        ...
                        <span class="text-sm font-medium text-[#E63946]">Konversi KBLI</span>
                    </div>
                </nav>
                ...
                <div class="mt-auto pt-8">
                    <h1 class="text-2xl font-extrabold leading-tight text-white sm:text-3xl lg:text-4xl">
                        Tabel Konversi KBLI<br class="hidden sm:block" />
                        <span class="text-white/90">KBLI 2020 × KBLI 2025</span>
                    </h1>
                    <p class="mt-4 max-w-xl text-sm leading-relaxed text-white/80 sm:text-base sm:leading-7">
                        Panduan penyesuaian kode lapangan usaha dari KBLI 2020 ke KBLI 2025 untuk kepentingan statistik maupun administrasi.
                    </p>
                    <div class="mt-6">
        ...
        <section class="py-[52px]">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col gap-8">
                ...
                <!-- ── Card Tabel ─────────────────────────────────────────── -->
                <div class="rounded-2xl border border-[#D9DAD8] bg-white p-6 sm:p-8 shadow-sm">

                    <!-- Header -->
                    <div class="flex items-start gap-3 mb-4">
                        <span class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-md bg-[#FAD9DA]">
                            <svg class="w-4 h-4 text-[#E63946]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                        </span>
                        <h2 class="text-[18px] font-bold text-[#1A1B18] pt-0.5">Tabel Konversi KBLI 2020 x KBLI 2025</h2>
                    </div>

                    <p class="text-[13px] leading-[22px] text-[#686964] mb-6">
                        Seiring dengan disempurnakannya KBLI 2020 menjadi KBLI 2025 yang telah mengacu pada struktur ISIC Revisi 5, diperlukan suatu panduan yang dapat menjembatani peralihan penggunaan klasifikasi tersebut. Publikasi Tabel Konversi KBLI 2020 ke KBLI 2025 ini disusun untuk memberikan acuan dalam melakukan penyesuaian kode dan klasifikasi lapangan usaha dari KBLI 2020 ke KBLI 2025, baik untuk kepentingan statistik maupun administrasi.
                    </p>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 mb-6">
                        ...
                        <div class="relative flex-shrink-0" ref="dropdownRef">
                            ...
                            >
                                ...
                                <svg class="h-4 w-4 text-[#686964]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            >
                                <button
                                    ...
                                    :key="opt"
                                    @click="filterKategori = opt; showDropdown = false"
                                    :class="[
                                        'w-full text-left px-4 py-2 text-[13px] hover:bg-[#FEF9F9] transition-colors',
                                        filterKategori === opt ? 'text-[#E63946] font-semibold' : 'text-[#1A1B18]'
                                    ]"
                                >{{ opt }}</button>
                            </div>
                        </div>
                    ...
                    <div v-if="isLoading" class="flex flex-col items-center justify-center py-16 gap-3">
                        ...
                        <p class="text-[13px] text-[#686964]">Memuat data konversi KBLI...</p>
                    </div>

                    <!-- Error -->
                    <div v-else-if="loadError" class="flex flex-col items-center justify-center py-16 gap-3">
                        <svg class="h-10 w-10 text-[#F87171]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <p class="text-[14px] font-medium text-[#686964]">Gagal memuat data</p>
                        <p class="text-[13px] text-[#9CA3AF]">Periksa koneksi Anda dan coba lagi</p>
                    </div>

                    <!-- Table -->
                    <template v-else>
                        <div class="overflow-x-auto rounded-xl border border-[#D9DAD8]">
                            <table class="w-full text-[13px]">
                                <thead>
                                    <tr class="bg-[#F3F4F6] border-b border-[#D9DAD8]">
                                        <th class="text-left px-5 py-3 font-semibold text-[#1A1B18] w-[130px]">KBLI 2020</th>
                                        <th class="text-left px-5 py-3 font-semibold text-[#1A1B18]">JUDUL KBLI 2020</th>
                                        <th class="text-left px-5 py-3 font-semibold text-[#1A1B18] w-[130px]">KBLI 2025</th>
                                        <th class="text-left px-5 py-3 font-semibold text-[#1A1B18]">JUDUL KBLI 2025</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr
                                    ...
                                    >
                                        <td class="px-5 py-4 font-medium text-[#1A1B18]">{{ row.kbl_2020?.trim() }}</td>
                                        <td class="px-5 py-4 text-[#282925]">{{ row.judul_kbl_2020?.trim() }}</td>
                                        <td class="px-5 py-4 font-medium text-[#1A1B18]">{{ row.kbl_2025?.trim() }}</td>
                                        <td class="px-5 py-4 text-[#282925]">{{ row.judul_kbl_2025?.trim() }}</td>
                                    </tr>
                                    <tr v-if="pagedData.length === 0">
                                        <td colspan="4" class="px-5 py-12 text-center text-[#9CA3AF] text-[13px]">
                        ...
                        <!-- Pagination -->
                        <div class="mt-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-[13px]">
                            <!-- Total rows -->
                            <span class="text-[#686964]">{{ totalRows.toLocaleString('id-ID') }} Row</span>
                            ...
                            <div class="flex items-center gap-2 text-[#686964]">
                                ...
                                <div class="relative">
                                    ...
                                    <svg class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 h-3 w-3 text-[#686964]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                                    </svg>
                                </div>
                            </div>
